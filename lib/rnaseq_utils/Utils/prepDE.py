...
parser.add_option('-i', '--input', '--in', default='ballgown',
                  help="the parent directory of the sample sub-directories or a textfile listing the paths to GTF files [default: %default]")
parser.add_option('-g', default='gene_count_matrix.csv',
                  help="where to output the gene count matrix [default: %default")
parser.add_option('-t', default='transcript_count_matrix.csv',
                  help="where to output the transcript count matrix [default: %default]")
parser.add_option('-l', '--length', default=75, type='int',
                  help="the average read length [default: %default]")
parser.add_option('-p', '--pattern', default=".",
...
if os.path.isfile(opts.input):
    ...
    try:
        ...
        for line in fin:
            if line[0] != '#':
                ...
                if len(lineLst) != 2:
                    print(f"Error: Text file with sample ID and path invalid ({line.strip()})")
                    exit(1)
                if lineLst[0] in samples:
                    print(f"Error: Sample ID duplicated ({lineLst[0]})")
                    exit(1)
                if not os.path.isfile(lineLst[1]):
                    print(f"Error: GTF file not found ({lineLst[1]})")
                    exit(1)
                samples.append(lineLst)
    except IOError:
        print(f"Error: List of .gtf files, {opts.input}, doesn't exist")
        exit(1)
else:
    # gtfList = False
    ...
    if not os.path.isdir(opts.input):
        parser.print_help()
        print(f"Error: sub-directory '{opts.input}' not found!")
        sys.exit(1)

    #####
    ## Collect all samples file paths and if empty print help message and quit
    #####
    samples = [(i, next(glob.iglob(os.path.join(opts.input, i, "*.gtf")))) for i in
               next(os.walk(opts.input))[1] if re.search(opts.pattern, i)]

if len(samples) == 0:
    parser.print_help()
    print(f"Error: no GTF files found under ./{opts.input} !")
    sys.exit(1)
...
def t_overlap(t1,
              t2):  # from badGenes: chromosome, strand, cluster, start, end, (e1start, e1end)...
        ...
        for i, v in enumerate(split):
            if is_transcript(v):
                ...
                try:
                    ...
                    print(f"Problem at line:\n:{v}\n")
                    print(f"i='{i}', len(v)={len(v)}")
                    sys.exit(1)
                geneIDs.setdefault(t_id, g_id)
                if not RE_STRING.match(g_id):
                    badGenes.append([v[0], v[6], t_id, g_id, min(int(v[3]), int(v[4])),
                                     max(int(v[3]), int(v[
                                                            4]))])  # chromosome, strand, cluster/transcript id, start, end
                    j = i + 1
...
if opts.cluster and len(badGenes) > 0:
    ...
    for u, c in enumerate(clusters):
        ...
        legend.append(
            list(itertools.chain.from_iterable([[my_ID], c])))  # my_ID, clustered transcript IDs
        for t in c:
            geneIDs[t] = my_ID
    ...
    try:
        ...
        for l in f:
            ...
            if v[2] == "transcript":
                ...
                transcript_len = 0
            if v[2] == "exon":
                transcript_len += int(v[4]) - int(
                    v[3]) + 1  # because end coordinates are inclusive in GTF
